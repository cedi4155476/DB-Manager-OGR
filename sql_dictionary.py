...
__author__ = 'Cédric Christen'
__date__ = 'June 2015'
__copyright__ = '(C) 2015, Cédric Christen'
...
geopackage_functions = [  # from www.gaia-gis.it/geopackage-2.3.0/geopackage-sql-2.3.0.html
...
def getSqlDictionary(spatial=True):
    ...
    if spatial:
        k += geopackage_keywords
        f += geopackage_functions
        c += geopackage_constants
...
def getQueryBuilderDictionary():
    ...
    foo = sorted(add_paren(ff( list(set.union(set(functions), set(geopackage_functions))) )))
